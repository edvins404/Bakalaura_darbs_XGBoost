...
# === SEED EKSPERIMENTA VĒRTĪBA ===
SEED = 6122003

RESULTS_FILE = Path(f"rezultati_kopsavilkums_seed{SEED}.xlsx")
...
# === BATCH APMĀCĪBAS FAILU SARAKSTS ===
BATCH_TRAIN_FILES = [
    # 1. grupa — pilna pazīmju kopa, mainīgs apjoms
    Path("merged_no_nan_clean_headers_no_duplicates_train.csv"),
    Path("stratificeta_atlase/merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct.csv"),
    Path("stratificeta_atlase/merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct.csv"),
    Path("stratificeta_atlase/merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct.csv"),
    Path("stratificeta_atlase/merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct.csv"),
    # 2. grupa — 10% pazīmes
    Path("stratificeta_atlase/10%/merged_no_nan_clean_headers_no_duplicates_train_grupa1_10pct.csv"),
    Path("stratificeta_atlase/10%/merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct_grupa1_10pct.csv"),
    Path("stratificeta_atlase/10%/merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct_grupa1_10pct.csv"),
    Path("stratificeta_atlase/10%/merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct_grupa1_10pct.csv"),
    Path("stratificeta_atlase/10%/merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct_grupa1_10pct.csv"),
    # 3. grupa — 50% pazīmes
    Path("stratificeta_atlase/50%/merged_no_nan_clean_headers_no_duplicates_train_grupa2_50pct.csv"),
    Path("stratificeta_atlase/50%/merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct_grupa2_50pct.csv"),
    Path("stratificeta_atlase/50%/merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct_grupa2_50pct.csv"),
    Path("stratificeta_atlase/50%/merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct_grupa2_50pct.csv"),
    Path("stratificeta_atlase/50%/merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct_grupa2_50pct.csv"),
]

MODEL_NAME_STRIP_PREFIX = "merged_no_nan_clean_headers_no_duplicates_train"

# === TESTA FAILU CEĻI ===
TEST_FULL  = Path(r"C:\Users\Edvins\Desktop\bakalaura_darbs\bez_bias_kolonnam_step2\merged\bez_NaN\Bez_dublikatu_galvenem\bez_dubl\merged_no_nan_clean_headers_no_duplicates_test.csv")
TEST_10PCT = Path(r"C:\Users\Edvins\Desktop\bakalaura_darbs\bez_bias_kolonnam_step2\merged\bez_NaN\Bez_dublikatu_galvenem\bez_dubl\testa kopas\merged_no_nan_clean_headers_no_duplicates_test_grupa1_10pct.csv")
TEST_50PCT = Path(r"C:\Users\Edvins\Desktop\bakalaura_darbs\bez_bias_kolonnam_step2\merged\bez_NaN\Bez_dublikatu_galvenem\bez_dubl\testa kopas\merged_no_nan_clean_headers_no_duplicates_test_grupa2_50pct.csv")

MODELS_DIR = Path(r"C:\Users\Edvins\source\repos\PythonApplication1\PythonApplication1\modeli_seed6122003")

# === BATCH TESTEŠANAS SARAKSTS: (modelis, testa_fails) ===
BATCH_EVAL_FILES = [
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_seed6122003.pkl",                               TEST_FULL),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct_seed6122003.pkl",              TEST_FULL),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct_seed6122003.pkl",              TEST_FULL),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct_seed6122003.pkl",              TEST_FULL),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct_seed6122003.pkl",              TEST_FULL),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_grupa1_10pct_seed6122003.pkl",                  TEST_10PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct_grupa1_10pct_seed6122003.pkl", TEST_10PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct_grupa1_10pct_seed6122003.pkl", TEST_10PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct_grupa1_10pct_seed6122003.pkl", TEST_10PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct_grupa1_10pct_seed6122003.pkl", TEST_10PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_grupa2_50pct_seed6122003.pkl",                  TEST_50PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_10pct_grupa2_50pct_seed6122003.pkl", TEST_50PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_25pct_grupa2_50pct_seed6122003.pkl", TEST_50PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_50pct_grupa2_50pct_seed6122003.pkl", TEST_50PCT),
    (MODELS_DIR / "merged_no_nan_clean_headers_no_duplicates_train_stratified_75pct_grupa2_50pct_seed6122003.pkl", TEST_50PCT),
]
...
# seed_suffix — ja norādīts, tiek pievienots modeļa faila nosaukumam (seed eksperimentam)
def train_model(csv_path: str, target_column: str, seed_suffix: str = "") -> None:
    stem = Path(csv_path).stem
    model_path = Path(f"{stem}{seed_suffix}.pkl")
    encoder_path = Path(f"{stem}{seed_suffix}_encoder.pkl")
...
# Apmāca visus modeļus no BATCH_TRAIN_FILES saraksta secīgi
def train_batch() -> None:
    missing = [f for f in BATCH_TRAIN_FILES if not f.exists()]
    if missing:
        print("Trūkst šādi faili:")
        for f in missing:
            print(f"  {f}")
        print("Pārtrauc batch apmācību.")
        return

    suffix = f"_seed{SEED}"
    total = len(BATCH_TRAIN_FILES)
    print(f"\nSāk batch apmācību: {total} modeļi | seed={SEED}\n")

    for index, csv_path in enumerate(BATCH_TRAIN_FILES, start=1):
        print(f"\n[{index}/{total}] {csv_path.name}")
        print("-" * 60)
        train_model(str(csv_path), "Label", seed_suffix=suffix)

    print(f"\nBatch apmācība pabeigta. Visi modeļi saglabāti ar sufiksu '{suffix}'.")


# Novērtē visus modeļus no BATCH_EVAL_FILES saraksta secīgi
def evaluate_batch() -> None:
    missing_models = [m for m, _ in BATCH_EVAL_FILES if not m.exists()]
    missing_tests  = list({t for _, t in BATCH_EVAL_FILES if not t.exists()})
    if missing_models or missing_tests:
        if missing_models:
            print("Trukst modeli:")
            for m in missing_models:
                print(f"  {m.name}")
        if missing_tests:
            print("Trukst testa faili:")
            for t in missing_tests:
                print(f"  {t}")
        print("Partrauc batch testesanu.")
        return
    total = len(BATCH_EVAL_FILES)
    print(f"\nSak batch testesanu: {total} modeli | seed={SEED}\n")
    for index, (model_path, test_csv) in enumerate(BATCH_EVAL_FILES, start=1):
        print(f"\n[{index}/{total}] {model_path.name}")
        print("-" * 60)
        evaluate_saved_model(str(test_csv), "Label", str(model_path))
    print(f"\nBatch testesana pabeigta. Rezultati saglabati: {RESULTS_FILE}")
...
def run_interactive_menu() -> None:
    print(f"\n=== XGBoost IDS Klasifikators (seed={SEED}) ===")
    print("  1. Apmācīt jaunu modeli")
    print("  2. Novērtēt saglabāto modeli")
    print("  3. Prognozēt (bez Label kolonnas)")
    print(f"  4. Batch apmācība ({len(BATCH_TRAIN_FILES)} modeļi, seed={SEED})")
    print(f"  5. Batch testēšana ({len(BATCH_EVAL_FILES)} modeļi, seed={SEED})")
    print("  0. Iziet")
    print()
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["train", "evaluate", "predict", "batch"], default=None)
